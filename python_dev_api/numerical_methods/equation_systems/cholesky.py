...
def cholesky(A, b):
    ...
    for k in range(n):
        sum1 = np.sum([L[k, p] * U[p, k] for p in range(0, k)])
        L[k, k] = U[k, k] = csqrt(A[k, k] - sum1)
        for i in range(k+1, n):
            sum2 = np.sum(np.fromiter((L[i, p] * U[p, k]
                                       for p in range(0, k)), dtype="complex"))
            L[i, k] = (A[i, k] - sum2)/U[k, k]
        for j in range(k+1, n):
            sum3 = np.sum(np.fromiter((L[k, p] * U[p, j]
                                       for p in range(0, k)), dtype="complex"))
...
def main():
    ...
    L, U, z, x = cholesky(matrix, vector)
    print("L =")
    print(L)
    print("U =")
    print(U)
