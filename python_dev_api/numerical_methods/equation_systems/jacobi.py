...
def jacobi(A, b, x0, tolerance, iterations):
    D = np.diag(np.diag(A))
    U = -1 * (np.triu(A) - D)
    L = -1 * (np.tril(A) - D)
    T, C = findProduct(D, U, L, b)

    count = 0
    dispersion = tolerance + 1
    print(spectralRadio(T))
    if spectralRadio(T) >= 1:
        raise Exception("Incorrect Spectra Radio")
...
def findProduct(D, U, L, b):
    D_inv = getInverseDiagonal(D)
...
def main():
    ...
    tolerance = 1e-7
    iterations = 100
    jacobi(matrix, vector, x0, tolerance, iterations)
