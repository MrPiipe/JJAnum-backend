...
def gaussEliminationPartialPivot(A, b):
...
def elimination(A, b):
    ...
    for k in range(0, n-1):
        augmented = partialPivot(augmented, n, k)
...
def partialPivot(augmented, n, k):
    maxNum = abs(augmented[k, k])
    maxRow = k
    for s in range(k+1, n):
        if abs(augmented[s, k]) > maxNum:
            maxNum = abs(augmented[s, k])
            maxRow = s
    if maxNum == 0:
...
def organizeMarks(x, marks):
    x_sorted = np.zeros(len(marks))
...
def main():
    ...
    x = gaussEliminationPartialPivot(matrix, vector)
    print(x)
